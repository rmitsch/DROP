...
import source.analysis.analyis_functions as af


if __name__ == '__main__':
    base_data_path: str = "/home/raphael/Development/data/TALE/evaluation/"
    ...
    for feature_set_to_use in feature_configs:
        cached_training_data_file_path: str = base_data_path + "results_" + feature_set_to_use + ".pkl"

        if force_recomputation_model or not os.path.isfile(cached_training_data_file_path):
            estimator, metrics, selected_feature_data, test_feats, test_labels = af.train(
                data=feature_data, cols_to_keep=feature_configs[feature_set_to_use]
            )

            with open(cached_training_data_file_path, 'wb') as handle:
                pickle.dump((estimator, metrics, selected_feature_data, test_feats, test_labels), handle)
        ...
        af.interpret_model(
            ...
            test_feats,
            test_labels,
            base_data_path + feature_set_to_use + "_"
        )
